function calculateNextSteps(square: string[][], field: string, ri: number, fi: number): number[][] | [] {
    ...
    const calculationForWhitePawns = (field: string, ri: number, fi: number) => 
        ri + 1 <= 7 ?
            new Array(checkIfPawnIsInital(ri, [ri + 2, fi]), checkIfFigureIsAhead([ri + 1, fi]) ? [] : [ri + 1, fi], checkIfPawnCanBeat('white', [ri + 1, fi + 1]), checkIfPawnCanBeat('white', [ri + 1, fi - 1])) :
            []

    const calculationForBlackPawns = (field: string, ri: number, fi: number) =>
        ri - 1 >= 0 ?
            new Array(checkIfPawnIsInital(ri, [ri - 2, fi]), checkIfFigureIsAhead([ri - 1, fi]) ? [] : [ri - 1, fi], checkIfPawnCanBeat('black', [ri - 1, fi + 1]), checkIfPawnCanBeat('black', [ri - 1, fi - 1])) :
            [] 

    const calculationForBishops = (square: string[][], field: string, ri: number, fi: number) => {
        ...
        const calculateRightUperConer = (square: string[][], field: string, ri: number, fi: number) => {
            let figureInTheWay = false 
            return Array.from({length: 8}, () => [ri, fi]).map((step, i) => {
                if((step[0] - i - 1) >= 0 && (step[1] + i + 1) <= 7 && !figureInTheWay) {
                    if(checkIfFigureIsAhead([step[0] - i - 1, step[1] + i + 1])) {
                        figureInTheWay = true
                        if(/2../.test(square[step[0] - i - 1][step[1] + i + 1]) && /1../.test(field) || /1../.test(square[step[0] - i - 1][step[1] + i + 1]) && /2../.test(field) ) {
                            return [step[0] - i - 1, step[1] + i + 1]
                        } else {
                            return []
                        }
                    } else {
        ...
        return [...calculateLeftUperConer(ri, fi), ...calculateRightUperConer(square, field ,ri, fi), ...calculateLeftLowerConer(ri, fi), ...calculateLeftRightConer(ri,fi)]
    }
    ...
    return (() => 
        ...
        /.3./.test(field) ? calculationForBishops(square, field, ri, fi)    :
        []
    )()
